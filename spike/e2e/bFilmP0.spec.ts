...
// Task 7: B-raw は B と同じ描き方（頂点共有・水面を後に描く）を three なしで書いたもの。
// 出力は b が b-film-p0.{json,md} のまま、braw が braw-film-p0.{json,md}（b の結果はコミット済みなので
// 再計測しない。-g "bFilmP0: braw" で braw だけ回す）
const CANDIDATES = ['b', 'braw'] as const
...
for (const candidate of CANDIDATES) {
  test(`bFilmP0: ${candidate} の 1cm の膜（平面・曲面）を pitch0 で測る（斜め視点の自己遮蔽と沈み込みを切り分ける基準）`, async ({
    page,
    context,
  }) => {
    ...
    for (const zfix of ZFIXES) {
      ...
      for (const view of P0_VIEWS) {
        ...
        if (row.measure.footprintPx === 0) {
          gaps.push(
            `film zfix=${zfix} z${view.zoom} ×${view.exaggeration}: footprintPx=0（画面外）`,
          )
    ...
    writeFileSync(
      new URL(`${candidate}-film-p0.json`, results),
    ...
    const body = [
      ...
      ...bowlLines,
      '',
      `${candidate} の曲面（すり鉢）の pitch0 の可視率の最小（未計測を除く）は **${minVisible(bowlRows).toFixed(4)}**。A/A' の bowl-film.md（zfix=offset、pitch0）の可視率の最小 0.9953/0.9955 と比べる。斜め視点（pitch60・85）の結果は未確定（本テストの対象外）。`,
      '',
      gaps.length === 0
        ? '未計測（footprintPx=0 または interiorPx=0）の行は無かった。'
        : `未計測の行（判定から除外し、ここに明記する）: ${gaps.join('、')}。`,
      '',
      `**カメラの視点についての注記（レビュー対応）**: ${candidate} は \`map.setTerrain\` を呼ばないため、MapLibre はカメラの位置・距離を平面の地図として計算する。${candidate} 自身が描く地形・水面は標高 × 倍率ぶん実際にはカメラへ迫り出しているため、範囲の中心（\`setView\` の既定）から外れた場所にある的（斜面の膜・すり鉢）を、高いズーム・高い倍率・pitch0（狭い視野）で見ようとすると視野の外へ出てしまう（05 で B 系を検討する際に考慮が要る、B・B-raw 共通の性質。B-raw は B とカメラの性質を共有する）。この計測では的の中心へカメラを寄せることで回避した`,
      '',
    ].join('\n')
    writeFileSync(
      new URL(`${candidate}-film-p0.md`, results),
      `# ${candidate} の 1cm の膜、pitch0 の追加計測\n\n${body}\n`,
    )
    console.log(body)
