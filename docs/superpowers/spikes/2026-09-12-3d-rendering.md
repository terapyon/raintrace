...
| 継ぎ目 | — | — | `seam.md`（境界の段差の表）: 合成は倍率1で最大 **20.0m**・平均12.5m（北の台地、基準からの高さ。`scenes.test.ts` の期待どおり）、倍率10で最大200.3m。実データは倍率1で最大24.2m。`b-seam.jpg`（縁あり/なし、z16・倍率1・10・pitch60・85）: 倍率1では北の台地の段差（20m）が小さく、縁あり/なしの見た目の差はほとんど無い。倍率10では、縁あり（skirt）は範囲の外周の四角形が高さ0（平面の地図の高さ）まで連続してつながり、境界が滑らかに見える。自己レビューでの追加確認（z15・倍率10・pitch45、成果物には含めていない）では、縁なし（skirt=0）は範囲の輪郭にごく薄いアンチエイリアスのにじみ（ハロー）が見える点だけが縁ありと異なった。地形のメッシュが範囲の外側で高さ0まで続かず途切れているためと考えられる。**縁（skirt）は範囲の境界を平面の地図と滑らかにつなぐために要る** | |
...
- `spike/results/sheets/b-synthetic.jpg`（Task 6）: 候補 B、合成の場面、`water: 'fixed'`、zfix=offset。A と同じ 64 視点。淡色地図のテクスチャ（`composeBasemap`、地理院の z17 のタイルを範囲の画素にそろえて合成）を地形に貼り、範囲の外は平面の地図（`gsi-pale`）のまま。オフラインの E2E ではベースマップの fixture（`tests/e2e/fixtures/tile.png`）が単色のため道路での位置合わせは目視できないが、範囲内の地形色（basemap × 陰影）と範囲外の平面の地図の色を実測すると（例: z16 ×1 p45 で外側 `(204,224,240)`・地形上面 `(189,208,222)` ≈ 外側 × シェード0.925）比例が一致しており、`composeBasemap` の原点（`originX`・`originY`）と `flipY=false` が正しく効いていることを確認した。倍率 ×10 でも池・膜の縁は地形とずれない（`spike/out/shots/b-synthetic-z16_×10_p60.png`）
- `spike/results/sheets/b-real.jpg`（Task 6）: 候補 B、実データの場面、`water: 'fixed'`（満水）、zfix=offset。同じ 64 視点。ほぼ全面が満水のため地形は見えないが、水面越しに**白い針状・点状のノイズ**が多数現れる（例 `spike/out/shots/b-real-z16_×1_p45.png`、右下に長い斜めの筋 1 本と細かい散点）。A の黒い針状ノイズ（範囲の外まで伸びる、原因は `tileBlockSampler` の端の clamp。§11）とは見え方が異なり、B の筋は範囲の中に留まり、色も白（地形・水面のどちらも描かれず背景の平面の地図が透けて見える色）である。これは Task 6 で追加した**無効セルの対策**（`maskedGridIndices`、無効セルにかかる四角形を地形・水面の両方で落とす）の効果と考えられる: 渋谷 500m の範囲の無効セルは全体の 0.19%（`real-scene.md`）と散在しており、そこにかかる三角形を落とすと地形にも水面にも小さな穴（背景が透ける隙間）が残る。A（MapLibre の raster-dem + Terrarium 経由）は無効セルを `?? 0` で埋めて連続なメッシュとして補間するため、この形の「穴」は現れず、代わりに標高の谷（の一部が黒い筋）が生じる。**B は「穴」、A は「谷」という異なる形で無効セルの扱いが表面化する**——どちらも本番（05）では無効セルをきちんと埋める・除外するなどの対策が要ることを示す（A の切り分け結果、§11 と合わせて申し送る）
...
- **B の無効セルの対策（Task 6）**: シェーダに無効セルの入力が無いため、`spike/src/gridMesh.ts` の `maskedGridIndices` で無効セルにかかる四角形（三角形 2 枚）を地形・水面の両方の index から落とした（`spike/src/candidates/b.ts`）。標高 0 へ落ちる巨大な尖り（スパイク）は防げたが、代わりに実データで**背景が透ける小さな穴**が地形・水面の両方に散在する（`b-real.jpg`。渋谷 500m の無効セルは全体の 0.19%）。05 では、穴を残すこの方式でよいか、無効セルを埋めてから描くか（A の `demFill=nearest` に相当）を検討する必要がある
- **B の縁（skirt）は範囲の境界の見た目に要る（Task 6）**: 縁が無いと、範囲の境界で地形のメッシュが宙で途切れ、輪郭にアンチエイリアスのにじみが出る（`b-seam.jpg`、§4「継ぎ目」）。05 で B 系を採る場合は縁つきのメッシュを既定にする
