...
**Task 5b の条件（先取り、fix round 2 で再訂正）**: A 系の曲面（すり鉢）で、pitch0（真上）の描画は8/8○（可視率最小0.9953）だが、これは polygonOffset(−1,−4) の深度の余裕（z15で概算cm単位。「数mm」ではない）が沈み込みを覆い隠している可能性を否定できない。レンダリングを介さない直接計算（`bowl-film-mesh.md`）で、MapLibre が実際に描く地形メッシュの高さとシミュレーションの標高の差 s（弦の高さ）を、地図ズームごとに実際に使われる DEM のズーム（地形タイルのズーム − deltaZoom 1。fix round 1 はこの1段のずれを見落としていた）で求めたところ、**地図ズーム15（DEMズーム14）で max s = 0.0998m、地図ズーム16（DEMズーム15）で 0.0246m——いずれも1cmを大きく超える。地図ズーム17（DEMズーム16）は 0.0059m、地図ズーム18（DEMズーム17）は 0.0015m で1cm未満**。斜め視点はさらに粗いタイル（demZoom 8〜16 相当）で描かれ、弦の高さは最大0.40mに達しうるが、レンダリングがそれを隠すかは可視率の指標では判定できず未確定。→ Task 10 の推奨は「曲面は地図ズーム17以上でのみ合格基準1が成立し、ズーム15・16は対策が要る。斜め視点・粗いタイルでの沈み込みは未確定のまま残る」と明記すること
...
- **A 系の曲面（すり鉢）は地図ズーム17以上で合格、ズーム15・16は沈み込みを確認（M1、`bowl-film.md`・Task 5b `bowl-film-mesh.md`。タスクレビュー・レビュー役の推奨で訂正、fix round 1・2 で再訂正）**: pitch60・85 の可視率の低下は、すり鉢の縁が視線より急な側で手前の台地に隠れる自己遮蔽で説明でき、`visibleRatio`（深度テスト無しの footprint との比）だけでは正当な遮蔽と沈み込みを区別できない。自己遮蔽が起きない pitch0 では A・A' とも 8/8 ○ で沈み込みは確認されなかった。**Task 5b では当初、沈み込まない高さの基準膜（20cm）との比 `ratio = visible(1cm)/visible(20cm)` で遮蔽を打ち消そうとしたが、fix round 1 のタスクレビューでこの比自体に欠陥がある（水面シェーダの discard による footprint の違い、持ち上げによる遮蔽の幾何の変化）と判明し、沈み込みゼロのモデルでも記録した16行の ratio を全て再現できるため、この比では判定できないと訂正した**（`bowl-film-ratio.md`）。代わりにレンダリングを介さず MapLibre の地形メッシュの高さを直接計算したところ（`bowl-film-mesh.md`）、当初（fix round 1）は地図ズームをそのまま DEM ズームとして使っており、**fix round 2 のタスクレビューで、地形タイルが実際に読む DEM は地形タイルのズームより deltaZoom（既定1、固定）だけ粗い（DEMズーム = タイルズーム − 1）と判明**。pitch0 で測り直すと、**地図ズーム15（DEMズーム14）で max s = 0.0998m（膜セルの79.5%が1cm超）、地図ズーム16（DEMズーム15）で 0.0246m（72.1%が1cm超）——いずれも不合格。地図ズーム17（DEMズーム16）は 0.0059m、地図ズーム18（DEMズーム17）は 0.0015mで合格**。斜め視点はさらに粗いタイル（demZoomがpitch60で12〜16、pitch85で8〜10相当）で描かれ、弦の高さは最大約0.40mまで拡大しうるが、レンダリングが実際にそれを隠すかは可視率の指標では判定できず未確定。さらに、pitch0 の8/8○も、polygonOffset(−1,−4) の深度の余裕（カメラ距離の2乗に比例する項を含み、z15・pitch0で概算cm単位——「数mm」ではない）が沈み込みを覆い隠している可能性を否定できない（逆に、本来隠れるべき遠い水面が薄い地形を透けて見える bleed-through が起きている可能性もあり、可視率の指標では検出できない）。05 で A 系を採る場合、次のような選択肢を検討する必要がある: (1) 水面の頂点を MapLibre の地形メッシュの高さに合わせる（A' 方式に近いが、MapLibre への問い合わせではなく本 spike の `meshHeightAt`（純粋関数。MapLibre 実装と誤差0で一致することを別レビューで確認済み）で、`getRenderableTiles` から求めた描画中の DEM ズームを使い自前計算する——A' の `queryTerrainElevation` のコストを避けられる）、水面の高さを `max(simElevation, meshHeightAt(...)) + depth` とする、(2) 弦の高さぶんのかさ上げ・深度バイアスを描画中の DEM ズームに応じて足す、(3) 粗いズームでは2D表示に切り替える。あわせて、polygonOffset の係数を選ぶときは近傍の弦の沈み込みと遠方の透けの両方を天秤にかける必要がある。本番の投影も考慮が要る: 本番の A は DEM5 を z15、DEM10B を z14 で描く想定であり、頂点間隔はそれぞれ約9.5m・19mとなるため、この bowl と同じ形状なら対角の弦の高さは約3.7cm・15cmに達する。**この結果だけでは B 系を優先する根拠にはならない**（B は曲面の斜め視点を未計測のまま）
