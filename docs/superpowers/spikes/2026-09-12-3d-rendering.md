...
**推奨: 候補 A**（MapLibre の 3D terrain ＋ シミュレーションの標高で高さを付けた水面の Custom Layer、`polygonOffset(−1, −4)`）。**条件: 実 GPU の fps で確定**（合格基準 3 の手動の結果はユーザーの計測待ち。ただし同じ GPU での参考値では、A は 1 つの視点で D15 の両方の条件を外しており、手動でも外す見込みが高い。下の「合格基準 3 の状況」と裁定 2）。

理由（規則ごとの評価は §8）:
1. A は合格基準 1（平面の 1cm の膜、zfix=offset で 8/8 ○）と合格基準 2（地形と水面に同じ倍率）を満たす。曲面は、描かれる地形タイルのズーム 16 以上なら幾何の上でも沈まない。描かれるタイル z15 以下では幾何の上で最大 2.46cm 沈み（p0 の描画では polygonOffset の偏りが覆う。斜め視点は未確定）、**A を採るならその対策が 05 の義務になる**（§10.1 の 1。05 の改訂で R05-4 として扱う）。B 系は水面と地形を同じ頂点で描くので、この沈み込みは構造上起きない
2. A 系と B 系がともに基準 1・2 を満たすので、規則 2 により A 系を採る。A 系には B のような標高 × 倍率の段差が無く、地形のメッシュとカメラは MapLibre が持つ（ただし DEM タイルの生成は自前で、範囲の中と外で DEM の出所が変わる境目は 05 で決める。§10.1 の 3・4）。B 系には構造上の弱点がある。B は `map.setTerrain` を呼ばないので、MapLibre はカメラを平らな地図として置き、B のメッシュは標高 × 倍率だけカメラへ迫り出す（z18 ×10 で約 200m、カメラの距離は約 437m〈960 × 600 の画面で計算した値〉。p85 では南の縁が 242m の壁になる）
...
**合格基準 3（fps）の状況**: 手動の結果はユーザーの計測待ち（§3 の手順、10 個の URL）。手動の計測は同じ開発機（GTX 1080 Ti）で行う見込みなので、同じ GPU の実 GPU・headless の参考値（60Hz が上限）がいちばん近い予測になる。その参考値では、B・B-raw が 60.0fps・長いフレーム 0%。A は z17 ×5 p60 では 58.0fps・長いフレーム 291 間隔のうち 2（0.7%）で満たすが、z16 ×10 p85 では 56.8fps（< 57）・285 のうち 6（2.1%。1% 以下なら 2 まで）で、D15 の両方の条件を外す。地形のみ（水面なし）の A も z16 ×10 p85 で 56.2fps・283 のうち 7（2.5%）なので、不足は地形の経路にあり、水面が足す render の CPU は約 0.2ms にとどまる。地形の経路のうちどれが原因か（この spike が `addProtocol` の中でメインスレッドで行う Terrarium への変換、重ねている hillshade、MapLibre の地形の描画そのもの）は切り分けていない（§3）。**手動の計測でも A は基準 3 を外す見込みが高い**ので、その場合の方針（裁定 2）も今お願いしたい。

**ユーザーに裁定してほしいこと**
1. **T9 の案（§9）**: A を採り、tech-spec §5.5 をこの案で改訂してよいか。裁定の後、05 の改訂（R05-3）で tech-spec §5.5 と spec 05 §4 を直す（D14）
2. **A が基準 3 を外した場合の方針**（見込みが高い分岐なので、今の裁定をお願いしたい。選択肢は §8 に並べるだけで、この報告はどれも推奨しない）: 原因の切り分け（最も安い一歩: 地形のタイルの生成をメインスレッドから外すか `encoding: 'custom'` を試し、hillshade なしで測る）／規則 1 のとおり B 系に移る／A を保ち、05 で地形のコストを見込む（粗いズームは 2D にする、地形を描く範囲を狭める など）／spec S §5 の代わりの方針（3D は地形のみ、または 3D を 05 から外す）
3. **RS-2: A の水面に three を使うか**: three を使う（gzip 126.5 KB。動的 import なので初期ロードは 425.1 KB のまま、総量は 692.0 KB で上限は 1.2 MB）か、生の WebGL2 で書く（`threeWater.ts` と同じ範囲で数えて、import を除き約 231 行。three の版は 142 行〈import を含めて 168 行〉で、約 90 行多い。回避策は確実 1〈後始末〉・判断 1〈カリング〉・念のため 1〈FLIP_Y/PREMULTIPLY〉）か。材料は §8 の規則 4 の後
...
**未計測として残すもの（Task 8 の裁定）**: (1) A' の地形のみの基準。A' は速度で 05 の候補から外れるので測っていない。A' の `pass=false` が水面と地形のどちらに由来するかは**未計測**。(2) A の長いフレームが決まったタイミングで繰り返し起きること（Task 8 のレビューの指摘）の、発生のタイミングと原因。地形のタイルの生成（この spike の `spikedem://`。`addProtocol` の中でメインスレッドで Terrarium に変換する）か、重ねている hillshade か、MapLibre の地形の描画かは切り分けておらず、**未計測**。地形のみの基準（`water=none`）も変換と hillshade を含むので、「地形のみ ≈ A」はこの 2 つを原因から外さない。
...
| 基準 1: 曲面（すり鉢）の 1cm の膜 | p0 の描画は 8/8 ○（最小 0.9953）。幾何の上では（直接の計算）描かれる地形タイル z16 以上で合格。z15 はメッシュが膜より最大 2.46cm 上に出る（p0 の描画では polygonOffset の偏り約 3cm がそれを覆う）。斜め視点は未確定 | p0 の描画は 8/8 ○（最小 0.9955）。斜め視点は A と同じく未確定 | p0 の描画は 8/8 ○（1.0000）。水面と地形が同じ頂点で、水面の高さは地形の高さ＋水深（`shaders.ts`）なので、弦による沈み込みは構造上起きない。斜め視点の描画は未計測 | B と同じシェーダ・同じ構造。p0 は B と全行一致。斜め視点の描画は未計測 |
...
個別の PNG（`spike/out/shots/`）はローカル生成物で git の管理外（`pnpm spike:e2e` で再生成する）。H に無いので、下と §4 ではコンタクトシートの該当のコマで示す。可視率 0.0000 の根拠は `water-b.md`。
...
- **基準 1**: A・B・B-raw は、平面の 1cm の膜で zfix=offset が 8/8 ○。A' は数値の計測が未取得（Playwright の 30 分の制限で打ち切り）。曲面は、p0 の描画では 4 候補とも 8/8 ○。A 系は幾何の上では（直接の計算）描かれる地形タイル z16 以上で沈まない。z15 はメッシュが膜より最大 2.46cm 上に出る（p0 の描画では polygonOffset の偏り約 3cm がそれを覆い 8/8 ○）。斜め視点（すり鉢の下のタイル 14〜18、最悪 0.0998m）は未確定。B 系の斜め視点の曲面は未計測。→ 平面では A・B・B-raw が合格。曲面では、A 系は描かれるタイル z15 以下で弦の沈み込みを持ち、**A を採るならその対策が 05 の義務になる**（§10.1 の 1。05 の改訂で R05-4 として扱う）。B 系は水面と地形を同じ頂点で描き、水面の高さは地形の高さに水深を足したもの（`shaders.ts` の `WATER_VERTEX` と `TERRAIN_VERTEX`）なので、弦による沈み込みは構造上起きない。B 系に残るのは深度の精度だけで、平面の膜（p60・85、offset 8/8）がそれを確かめている。B 系の斜め視点の曲面の描画は未計測
...
- **基準 3**: **ユーザーの計測待ち**。参考値からの見込みは次のとおり
  ...
  - A は z16 ×10 p85 で D15（平均 57fps 以上かつ長いフレーム 1% 以下）の両方の条件を外す: 平均 56.8fps（< 57）、長いフレーム 285 間隔のうち 6（2.1%。1% 以下なら 2 まで。ヒストグラム 276/3/5/1/0）。z17 ×5 p60 は 58.0fps・291 のうち 2（0.7%）で満たす。A' もほぼ同じ（z16 ×10 p85 で 56.6fps・284 のうち 6）
  - 手動の計測は同じ開発機（GTX 1080 Ti）で行う見込みで、しかも地理院のタイルを実際に取りに行くので、headless の参考（fixture で応答）より条件が良くなる理由は無い。参考値がいちばん近い予測で、**手動でも A は外す見込みが高い**
  - 地形のみの A（水面なし）も z16 ×10 p85 で 56.2fps・283 のうち 7（2.5%）。足りない分は地形の経路のコストで、水面の render の CPU は約 0.2ms（A 0.19〜0.22ms）。ただし地形のみの基準も Terrarium への変換と hillshade を含むので、この一致は変換と hillshade を原因から外さない。原因の候補は (i) この spike が `addProtocol` の中でメインスレッドで行う Terrarium への変換（`a.ts` の `registerProtocol`。本番の A も GSI の PNG を `addProtocol` で変換するので、原因ならそのまま本番に持ち込まれる。Worker に移すか `encoding: 'custom'`〈§11 (a)〉で消せる可能性がある）、(ii) 重ねている hillshade（地形への追加の描き込みのコスト。本番では要らないかもしれない）、(iii) MapLibre の地形の描画そのもの（A にとって構造上のもの）。**この報告は原因を決めない**
  - SwiftShader の値（A 2.8〜3.0fps、B 4.7〜4.9fps）はソフトウェア描画なので、絶対値に意味は無い
- → 基準 3 が確定するまで、4 候補とも暫定。**推奨には「実 GPU の fps で確定」の条件を付ける**。見込みは A が外すほうに傾いている（次の分岐）

**A が基準 3 を外した場合**（見込みが高い分岐。ユーザーに今の裁定をお願いしたい。選択肢を並べるだけで、この報告はどれも推奨しない）:
- 原因の切り分け（最も安い一歩）: 地形のタイルの生成をメインスレッドから外す（Worker）か `encoding: 'custom'` を試し、hillshade なしで同じ 2 視点を測る。原因が変換か hillshade なら直せる見込みがある。MapLibre の地形の描画そのものなら、A にとって構造上のもの
- 規則 1 をそのまま当てはめると A 系が外れ、基準 3 を満たした B 系が推奨の候補に上がる。その場合、B のカメラの弱点（規則 2）、縁（skirt）の要否が未確定であること、ベースマップの合成を 05 で引き受けることになる。一方、曲面の弦の沈み込みは B 系では構造上起きない
- A を保ち、05 の設計で地形のコストを見込む。例: 粗いズームは 2D に切り替える、3D の地形を描く範囲を狭める（範囲とその周りだけ。ただし範囲の外との境目に継ぎ目が戻り、規則 2 の 1 つ目の理由が弱まる）。どれが効くかはこの spike では測っていない
- spec S §5 の代わりの方針: 案 1「PoC の 3D は地形のみ（A の地形）とし、水深は 2D 表示で見せる」、案 2「3D を 05 から外し、2D の表示で PoC を終える」。ただし地形のみの A も同じ fps なので、原因が変換や hillshade で、それを直せる場合を除き、**案 1 では基準 3 の不足は解けない**

### 規則 2: A 系と B 系がともに合格なら A 系

基準 1・2 は両系とも満たす（基準 3 は規則 1 の条件のとおり）。A 系を採る。
1. B のような標高 × 倍率の段差が範囲の境界に無い。B は境界に最大 20.0m（合成）・24.2m（実データ）× 倍率の段差がある（`seam.md`）。縁の要否は未確定。A でも、範囲の中と外で DEM の出所が変わる境目は残り、その扱いは 05 で決める（§10.1 の 4）
2. 地形のメッシュとカメラは MapLibre が持つ。B はベースマップの合成、縁、基準の標高、無効セルのマスクを自前で持ち、05 ではクリックからセルを求めるレイの交差も自前になる（spec 05 §4）。ただし A も DEM タイルの生成は自前（Terrarium への変換、GSI に無いズーム、範囲の中は 02 のグリッドから全ズーム。§10.1 の 3・4）
3. **B 系の構造上の弱点**: B は `map.setTerrain` を呼ばないので、MapLibre はカメラを平らな地図として置き、B のメッシュは標高 × 倍率だけカメラへ迫り出す（z18 ×10 で約 200m、カメラの距離は約 437m〈960 × 600 の画面で計算した値〉。p85 では南の縁が 242m の壁）。real・z17 ×10 p85 では縁の壁が池を覆って可視率 0.0000、報告の画像でも z17 ×10 p60 で B の画面は手前の膜で埋まる（§7.1 の 3）。A では MapLibre がカメラを地形の高さ（`getCenterElevation`）に合わせるので、この問題は起きない。B の対策の案（範囲の中心のセルの標高を高さ 0 にする）は未検証で、起伏 × 倍率の分は残る

**曲面で残る差（規則 2 の天秤の反対側）**: B 系は水面と地形を同じ頂点で描くので、弦の沈み込みを構造上持たない。A はその対策（§10.1 の 1）を 05 で負う。規則 2 の結論（A）は変わらないが、B 系へ移るかを裁定するときの材料になる。
...
- 注: 差 4 は閾値（3）を 1 件だけ外れている。B-raw の GL の状態の 4 件のうち 2 件（`UNPACK_COLORSPACE_CONVERSION_WEBGL`〈単色の fixture では見え方に出ない〉と、再アップロード時の FLIP_Y/PREMULTIPLY〈MapLibre 6.6.0 が `render` の前に既定に戻す〉）は、画像が変わることを観測していない。差は数え方の粒度には頑健だが、備えを回避策に数えるかどうかには頑健でない（この 2 件を数えなければ差は 2）
...
A の水面は今は three で書いている（`threeWater.ts`）。水面は 1 つのメッシュと 2 つのテクスチャ（標高・水深）だけなので、three なしで書いた場合の量を B-raw の水面の部分から見積もった。数える範囲は `threeWater.ts` がしていることにそろえた（最終レビューの指摘で、枠組みの分を足して数え直した）。

**(a) 行数: import を除いて約 231 行。three の版（`threeWater.ts`）は import を除いて 142 行（全体は 168 行、うち import 26 行）なので、約 90 行多い**。142 行は A' 用の `elevScale`・`setElevation` を含むので、A だけの three の版と比べれば約 100 行多い。どちらも spike のコードなので、おおよその比較。
- 描画と GL の部分（`braw.ts`、H の版の行番号。計 126 行）:
  ...
  - 216〜229 から、地形のプログラム（219）と地形の index バッファ（225）を除いた 12 行（後始末）
- `threeWater.ts` も持つ枠組み（`braw.ts`、計 40 行）: 52〜54（mount の先頭と格子の大きさ）、56（render の時間の記録）、60〜69（標高・水深のコピー、状態、座標の換算、モデル行列）、75（GL の状態の変数）、77〜80（レイヤーの先頭）、230〜231（レイヤーの閉じと追加）、233〜251（倍率・水深・表示の切り替えのハンドル）
- `glProgram.ts` 65 行（シェーダのコンパイルとリンク、uniform の場所、R32F のテクスチャの作成。水面だけでも全部要る）
- 合計 126 + 40 + 65 = 231

**(b) 回避策: B-raw の GL の状態の 4 件のうち、A の水面に当てはまるのは 3 件。確かさで分けると、確実 1・判断 1・念のため 1**
- **確実: 明示的な後始末**。プログラム、VAO、頂点と index のバッファ、2 つのテクスチャ、リンク後のシェーダを自分で消す。three では `dispose()` が肩代わりする。B-raw では当初 2 つ漏れていた
- **判断: カリング**。MapLibre 6.6.0 は Custom Layer の `render` の前に `setCustomLayerDefaults` で CULL_FACE を既定（無効）に戻す（`maplibre-gl-dev.mjs` 17040〜17047 行）。three は既定の `side: FrontSide` で裏面を落とすので、生で書くなら裏面をどうするかを自分で決める。B-raw で実際に踏んだ罠は、B が自前で描く地形の裏面が水面を隠したことだった。A の地形は MapLibre が描くので同じ罠にはならず、A の水面の裏面を描いたときに見え方が変わるかは確かめていない
- **念のため: 再アップロード時の `UNPACK_FLIP_Y_WEBGL`・`UNPACK_PREMULTIPLY_ALPHA_WEBGL`**。水深を毎フレーム同じ経路で再アップロードするので形の上では当てはまるが、MapLibre 6.6.0 は `render` の前にこの 2 つも既定に戻す（同じ箇所）。凡例（素直に書くと誤った絵・エラーになる問題を避ける対策）に照らせば、回避策というより、他のコードが値を変えた場合への備え。FLIP_Y が R32F に効くかは確かめていない（§11）
- **当てはまらない: `UNPACK_COLORSPACE_CONVERSION_WEBGL`**。この設定は画像・canvas からのアップロードにだけ効く。B-raw ではベースマップの canvas のために設定した。A の水面は `Float32Array` を R32F のテクスチャに上げるだけで、ベースマップを持たない
...
**(c) バンドル**: three のチャンクは gzip で 126.5 KB。動的 import にすれば初期ロードは 425.1 KB（04 の最終の実測）のまま変わらず、総量は 565.5 + 126.5 = 692.0 KB（tech-spec §14.2 の上限 1.2 MB）。three を採らなければ 126.5 KB が無くなる（B − B-raw = 125.6 KB）。
...
→ **比べるもの**: three を使う場合は、126.5 KB（動的 import なので初期ロードは変わらない）と依存 1 つ（tech-spec §2 原則 3）。使わない場合は、import を除いて約 231 行（three の版より約 90 行多い）と、回避策の確実 1（後始末）・判断 1（カリング）・念のため 1（FLIP_Y/PREMULTIPLY）、MapLibre の内部の既定への暗黙の依存。どちらを採るかはユーザーの裁定（RS-2）。
...
D14 のとおり、tech-spec 自体はこの PR では変えない。ユーザーの裁定の後、05 の改訂（R05-3）で反映する。R05-3 の改訂は、05 のブランチ（04 の上）の tech-spec に対して行う。

**この案は、A が実 GPU の fps で合格基準 3 を満たした場合のもの**。外した場合は §1 の裁定 2 に従って書き直す。推奨（A）に合わせた文面は次のとおり。`<…>` はユーザーの裁定で埋める。
...
tech-spec の箇所は、行番号ではなく節の名前と引用で示す（行番号は版で変わり、この PR の base の `main` と 04 の先頭 9be4ec3 でも違う）。

**§5.5 を決めると、RS-2 の裁定によらず変わる箇所**
- §5.5「地形の描画方式（未決。Phase 1 の最初にスパイクで確定）」: 見出しと本文を上の改訂案で置き換える（候補の表の「Custom Layer 内の Three.js」を含む）
- §17 未決事項の表の行「地形の描画方式 | Phase 1 の最初 | §5.5 のスパイクで確定する」: 決まったので外す
- §18 決定事項の要約の 13「地形の描画方式は Phase 1 の最初にスパイクで確定する（§5.5）」: 決定の内容に改める
- §20 実装 spec へ引き継ぐ事項の「Phase 1 の範囲」の行（「3D は §5.5 のスパイクとして並行させ」）: スパイクの結論を指すように改める

**three を採らない場合に、あわせて変わる箇所**（three を採る場合は変えない。§14.2 の総量の見込みは 692.0 KB = 04 の実測 565.5 KB + 126.5 KB）
- §1 技術スタック一覧の「3D 描画 | Three.js | MapLibre Custom WebGL Layer 内で使用。地形の描画方式は §5.5」
- §1.1 採用しない技術と理由: react-map-gl の「MapLibre Custom WebGL Layer に Three.js を差し込む設計」、react-three-fiber の「Custom Layer 内では Three.js を直接扱う方が単純」
- §4.1 ディレクトリ構成の「renderer/ Three.js / WebGL。React には非依存」
- §4.2 レイヤー境界の強制の「Simulation Engine を MapLibre や Three.js に依存させない」（base-spec §61 の引用。文面は残し、dependency-cruiser の規則から three を外すかを 05 で決める）
- §5.1 三層構造の図の「(Three.js / Custom Layer)」、§5.3 MapLibre と React の接続のコード例のコメント「Three.js をこの中で使う」
- §14.2 バンドルサイズの「Three.js、シミュレーションを含む」「Three.js および `src/renderer/`」と、04 の版にある「05 で動的 import する Three.js」
- §18 決定事項の要約の 5「MapLibre と Three.js は React の外で命令的に扱う」
- `vite.config.ts` のコメント「05 で動的 import する Three.js なども」（04 のブランチにあり、`main` には無い）
- base-spec の Three.js の記述（技術の一覧の 2 か所と、「MapLibreやThree.jsに依存させない」）。base-spec を直すかはユーザーの判断
...
1. **水面の置き方**（A を採るなら 05 の義務。05 の改訂で R05-4 として扱う）: 曲面では、描かれる地形タイルのメッシュが弦の高さぶん膜より上に出る（下の表）。選択肢:
   - (a) 水面の高さを `max(sim, meshHeightAt(描かれる地形タイルのズーム)) + depth` にする。`meshHeightAt` は純粋関数（MapLibre の実装・数値の複製・実際の頂点の高さの 3 通りで誤差 0 を確認済み）で、描かれるタイルのズームは `getRenderableTiles` から求める。A' の `queryTerrainElevation` のコストを避けられる。ただし (a) では弦の分だけ水面がシミュレーションの標高から上にずれる（描かれるタイル z15 で最大 2.46cm × 倍率。A' の食い違い 0.024m と同じ大きさ）。(a) を採るなら、A と A' の差は高さの取り直しのコストだけになる
...
- 表の z16〜z18 の弦の高さは、z15 の値からの 1/4 の規則（間隔が半分なら弦は 1/4）による。spike の直接の計算（`bowl-film-mesh.md`、p0）は z16 0.59cm・z17 0.15cm・z18 0.07cm で、z18 は表の 0.04cm より大きい（どれも 1cm より十分小さい）
...
- **(e) 実 GPU の fps（合格基準 3）**: ユーザーの計測待ち（§3）。あわせて未計測: A' の地形のみの基準、A の長いフレームが決まったタイミングで繰り返し起きることの原因（メインスレッドでの Terrarium への変換、hillshade、MapLibre の地形の描画のどれか。地形のみの基準も変換と hillshade を含むので、それらを原因から外さない）
...
- `UNPACK_FLIP_Y_WEBGL` が R32F（`ArrayBufferView` からの `texSubImage2D`）に効くかは確かめていない。WebGL の仕様（6.10）は、以後のすべての `texImage2D`・`texSubImage2D` に効くと書いているので、H の `braw.ts` のコメント（「R32F の texSubImage2D 自体には無関係」）を 05 に持ち込まない
- 計画の Task 8 Step 6 に残る手動の計測の手順は改訂前の版（8 個の URL）。使うのは §3 の「§9 改訂版」（10 個の URL）。計画にも注を足した
