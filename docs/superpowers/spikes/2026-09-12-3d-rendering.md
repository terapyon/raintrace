...
**Task 5b の条件（先取り、fix round 1 で訂正）**: A 系の曲面（すり鉢）は pitch0（真上）で 8/8 ○（合格）。斜め視点は、沈み込まない高さの基準膜（20cm）との比では判定できないと判明した（`bowl-film-ratio.md`。足跡の差＝縁の discard と、持ち上げによる遮蔽の幾何の変化で説明がつき、沈み込みゼロのモデルで記録した全 16 行を再現できる）。代わりにレンダリングを介さない直接計算（`bowl-film-mesh.md`）で、すり鉢の代表点まわりの地形メッシュの高さとシミュレーションの標高の差 s を求めたところ、**ズーム15で max s = 0.0246m（膜セルの 72.1% が 1cm の膜厚を超え、実際に沈み込む）、ズーム16は 0.0059m、ズーム17は 0.0015m でいずれも 1cm 未満（合格）**。→ Task 10 の推奨は「曲面はズーム16以上で合格基準1を満たし、ズーム15は沈み込みへの対策が要る」と明記すること
...
- **A 系の曲面（すり鉢）はズーム16以上で合格、ズーム15は沈み込みを確認（M1、`bowl-film.md`・Task 5b `bowl-film-mesh.md`。タスクレビュー・レビュー役の推奨で訂正、fix round 1 で再訂正）**: pitch60・85 の可視率の低下は、すり鉢の縁が視線より急な側で手前の台地に隠れる自己遮蔽で説明でき、`visibleRatio`（深度テスト無しの footprint との比）だけでは正当な遮蔽と沈み込みを区別できない。自己遮蔽が起きない pitch0 では A・A' とも 8/8 ○ で沈み込みは確認されなかった。**Task 5b では当初、沈み込まない高さの基準膜（20cm）との比 `ratio = visible(1cm)/visible(20cm)` で遮蔽を打ち消そうとしたが、fix round 1 のタスクレビューでこの比自体に欠陥がある（水面シェーダの discard による footprint の違い、持ち上げによる遮蔽の幾何の変化）と判明し、沈み込みゼロのモデルでも記録した16行の ratio を全て再現できるため、この比では判定できないと訂正した**（`bowl-film-ratio.md`）。代わりにレンダリングを介さず MapLibre の地形メッシュの高さを直接計算したところ（`bowl-film-mesh.md`）、**ズーム15で max s（メッシュ高さ−シミュレーション標高）= 0.0246m（膜セルの72.1%が1cmの膜厚を超える）、ズーム16は0.0059m、ズーム17は0.0015mで、ズーム15だけが1cmを超え実際に沈み込む**。05 で A 系を採る場合、ズーム15（およびそれに相当する粗い DEM。視野内の最も粗いタイルは pitch85で更にズーム9〜11相当まで下がる）での沈み込みへの対策が必要。**この結果だけでは B 系を優先する根拠にはならない**（B は曲面の斜め視点を未計測のまま）
