...
**Task 5b の条件（先取り）**: A 系の曲面（すり鉢）での合格基準1は pitch0（真上）でのみ確認できている。斜め視点（pitch60・85）は、沈み込まない高さの基準膜（20cm）との比で沈み込みを直接確認しており（`bowl-film-ratio.md`、16 視点中 11 行が ×、特に倍率10）、Task 10 の推奨はこの条件（斜め視点・高倍率での対策が要る）を明記すること
...
- **A 系の曲面（すり鉢）は pitch0 でのみ合格、斜め視点は沈み込みを確認（M1、`bowl-film.md`・Task 5b `bowl-film-ratio.md`。タスクレビュー・レビュー役の推奨で訂正）**: pitch60・85 の 8 行中 5 行の × は、すり鉢の縁が視線より急な側で手前の台地に隠れる自己遮蔽で説明でき、`visibleRatio`（深度テスト無しの footprint との比）だけでは正当な遮蔽と沈み込みを区別できない。自己遮蔽が起きない pitch0 では A・A' とも 8/8 ○ で沈み込みは確認されなかった。**Task 5b で、沈み込まない高さの基準膜（20cm）との比 `ratio = visible(1cm)/visible(20cm)` を取り遮蔽を打ち消したところ、A の MEASURE_VIEWS 16 視点中 11 行が ×（ratio < 0.98。倍率10 は z15〜z18 の 8 行すべて、倍率1 も z17・z18 の一部）で、斜め視点で実際の沈み込みが起きていることを確認した**（最小 ratio 0.7551、z18・倍率10・pitch85）。05 で A 系を採る場合、斜め視点・高倍率での沈み込みへの対策（LOD の精度を上げる、あるいは倍率を制限するなど）が必要。**この結果だけでは B 系を優先する根拠にはならない**（B は曲面の斜め視点を未計測のまま）
