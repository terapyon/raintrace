...
範囲の決め方は spec 02 §3 のとおり `origin = floor(pc − N/2)`。この式では、クリックした地点は必ずセル (N/2, N/2)（N が偶数のとき、幾何学的な中心の南東のセル）に入り、**そのセルの中心から半セル以内** にある。範囲の幾何学的な中心からは最大 1 セルずれる。spec §3 の「範囲の中心は、クリックした地点から半セル以内」は、この意味で読む（`.handoff` に記録し、レビュー役に確かめる）。
...
describe('computeGridRange', () => {
  ...
  it.each(points)('経度 %f・緯度 %f をクリックすると、その地点はセル (N/2, N/2) に入る', (lon, lat) => {
    const range = computeGridRange(lon, lat, 500, 17)
    const half = Math.floor(range.size / 2)
    expect(cellAt(range, lon, lat)).toEqual({ col: half, row: half })
    const position = gridPositionM(range, lon, lat)
    // セル (N/2, N/2) の中心から半セル以内
    expect(Math.abs(position.x - (half + 0.5) * range.cellSizeM)).toBeLessThanOrEqual(range.cellSizeM / 2)
    expect(Math.abs(position.y - (half + 0.5) * range.cellSizeM)).toBeLessThanOrEqual(range.cellSizeM / 2)
  })
...
export function computeGridRange(lon: number, lat: number, sizeM: number, z: number): GridRange {
  ...
  return {
    ...
    cellSizeM,
    size,
    originX: Math.floor(center.x - size / 2),
    originY: Math.floor(center.y - size / 2),
    center,
  }
}
