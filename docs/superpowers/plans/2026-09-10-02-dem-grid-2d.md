...
範囲の原点は `origin = round(pc − N/2)` とする（計画のレビューで、spec 02 §3 の floor から改めた。floor では範囲の中心が最大 1 セルずれ、spec の「範囲の中心は、クリックした地点から半セル以内に来る」と両立しない。spec 02 §3 も直した）。pc − origin は [N/2 − 0.5, N/2 + 0.5) に入るので、範囲の中心（origin + N/2）はクリックした地点から半セル以内に来る。クリックした地点は、N が偶数なら列・行が N/2 − 1 か N/2、奇数なら (N − 1)/2 のセルに入る（那覇付近の z17 では N = 467 の奇数になる）。
...
01 のテスト「ping のたびに id が増える」の `worker.posted.map((m) => m.id)` は、`MainToWorkerMessage` に `loadTerrain`（`requestId` を持つ）が加わると型が合わなくなるので、`worker.posted.flatMap((m) => (m.type === 'ping' ? [m.id] : []))` にする（Task 5 のコミットで直す）。
