...
export class TerrainOverlay {
  ...
  /**
   * 覚えている地形（this.terrain）と表示の設定（this.display）で、ソース・レイヤー・画像を足す。
   * showTerrain と restore（ベースマップの切り替えの後）から呼ぶ。待つ間に表示の設定が変わりうるので、
   * 描く時点で覚えている設定を使う（showTerrain・setDisplay が先に代入するので、地形があれば null ではない）
   */
  private addAll(): void {
    ...
    // レイヤーを足した後なので、下の setDisplay の条件（標高のレイヤーがある）を満たして適用される
