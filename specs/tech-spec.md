...
- 06 へ引き継ぐ条件: 05 の最終では 8 条件すべてが D15 を満たす。残るのは、1000 m で矢印の間隔に 5 m を選んだとき（実効 10 m・10,000 本。c25be04 の既定と同じ本数）の落ち方で、未計測（51〜55 fps の見込み）。06 の M3 で矢印の本数を変えて測る。タイルの組み立て（24 ラン全部を見ても最大 15.7 ms）と水面の render CPU（0.416 ms）は、05 の記録の時点で原因から外れている
...
| 地図操作時のフレームレート | D15 | 3 地点 × 4 視点 × 地形のみ・水面ありの 24 セルすべて 60.0 fps（ランの最小 59.8）、長いフレーム 0、g2 最大 2。雨は 500 mm・半径 50 m、矢印は既定の本数。みなとみらい 1000 m は段 2 | 基準内（1000 m で矢印 5 m を選んだときは 06 の M3 で測る） |
