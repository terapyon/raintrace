...
| 指標 | 目標 | 測った値（最悪の条件） | 判定 |
|---|---|---|---|
| 1 step の所要時間（中央値） | < 8ms | 8.60 ms（綾瀬 500 m・全面を濡らす雨・2D。渋谷 8.20・みなとみらい 8.30。M2 比 −9〜−17%。1000 m は参考で 30.80・36.00 ms） | 超過（500 m。近傍の添字を直した Task 15 の後もわずかに超える。ブロック単位の走査〈Task 24〉の効果は ≤ 1〜2% と分かり実装せず） |
