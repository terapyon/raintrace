...
| 指標 | 目標 | 測った値（最悪の条件） | 判定 |
|---|---|---|---|
| 1 step の所要時間（中央値） | < 8ms | 8.60 ms（綾瀬 500 m・全面を濡らす雨・2D。渋谷 8.20・みなとみらい 8.30。M2 比 −6.8%〜−9.5%。Task 15 自身の前後〈同日、`until=window` の別の計測〉は −17.3%〜−18.8%で M2 比とは基準が異なる。1000 m は参考で 30.80・36.00 ms、M2 比 −17〜−19%） | 超過（500 m。近傍の添字を直した Task 15 の後もわずかに超える。ブロック単位の走査〈Task 24〉の効果は ≤ 1〜2% と分かり実装せず） |
| 1 step の所要時間（p95） | < 16ms | Task 15 は 1 回の測定でばらつきが大きく中央値のみで判定（`docs/perf/2026-09-17-fixes.md` Task 15） | 参考（判定は中央値で行った） |
...
| 1000m 四方 | 上の基準をすべて満たすか | M4 で最長ブロック・止まっている間の再描画・再生中の割り当て・fps は基準内になった。なお外れるのは 1 step（参考）と平衡（綾瀬 r10 80.2 s、r100 は綾瀬・渋谷とも届かず） | 超過。500 m も平衡の基準を満たさないため、1000 m に固有の問題ではなく fill-spill-merge の対応に集約される。1000 m を外すか注意を出すかはユーザーの裁定に上げる（M6・Task 29） |

1 番目（1 step）の未達は残り、Rust WASM 移行検討（§6.3）の判断は M6・Task 29 に持ち越す。2 番目（メインスレッドの最長ブロック時間）は M4 で解消した。
