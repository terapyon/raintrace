...
- 06 へ引き継ぐ条件: 05 の最終では 8 条件すべてが D15 を満たす。残るのは、1000 m で矢印の間隔に 5 m を選んだとき（実効 10 m・10,000 本。c25be04 の既定と同じ本数）の落ち方。**06 の M3（2026-09-17、`docs/perf/2026-09-17-isolation.md`）で計測**: 55.7（49.1〜57.1）fps・g2 41（29〜107）で D15 を外し、見込みどおり c25be04 の 55.3 に近い値に落ちた。矢印なし・矢印 20 m（625 本）・既定（2,500 本）はいずれも 60.0 前後で、fps は矢印の本数とともに下がる（主因は矢印の symbol の本数。転送・水面の描画・地形の描画は既定の本数では上限の内）。タイルの組み立て（24 ラン全部を見ても最大 15.7 ms）と水面の render CPU（0.416 ms）は、05 の記録の時点で原因から外れている
...
| 地図操作時のフレームレート | D15 | 3 地点 × 4 視点 × 地形のみ・水面ありの 24 セルすべて 60.0 fps（ランの最小 59.8）、長いフレーム 0、g2 最大 2。雨は 500 mm・半径 50 m、矢印は既定の本数。みなとみらい 1000 m は段 2 | 基準内（既定の本数）。**06 の M3（2026-09-17）で測定**: 1000 m で矢印 5 m を選んだとき（実効 10 m・10,000 本）は 55.7（49.1〜57.1）fps で D15 を外す。主因は矢印の symbol の本数で、M4 の対応はユーザーの裁定待ち |
