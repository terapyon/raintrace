...
| 地図操作時のフレームレート | D15 | 3 地点 × 4 視点 × 地形のみ・水面ありの 24 セルすべて 60.0 fps（ランの最小 59.8）、長いフレーム 0、g2 最大 2。雨は 500 mm・半径 50 m、矢印は既定の本数 | 基準内（1000 m で矢印 5 m を選んだときは 06 の M3 で測る） |
| 地点クリックから 2D の地形表示まで | < 3秒 | 0.77 s（綾瀬 1000 m の 3 回の中央値。1 ランの最大 0.82 s） | 基準内 |
...
- 初期ロードと `map`・`ui`・`index`・`three`・`View3d`・`waterLayer` は 05 と同じ。`simulation.worker` が 8.7 → 8.8 KB、総量が 702.2 → 702.4 KB（06 の M1 の Worker の変更のぶん。計測のコードは通常のビルドに残らないことを確かめている）
