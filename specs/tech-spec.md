...
| 範囲・視点（括弧は実測） | 地形のみ（05 の記録、c25be04） | 水面あり（05 の記録、c25be04） | 地形のみ（05 の最終 b3a8916） | 水面あり（05 の最終 b3a8916） |
|---|---:|---:|---:|---:|
| 500 m z17 ×5 p60（pitch 60.00・描かれるタイル 17） | 60.0 fps | 60.0 fps | 60.0 fps | 60.0 fps |
| 500 m z16 ×10 p85（pitch 78.60・タイル 16） | 60.0 fps | 60.0 fps | 60.0 fps | 60.0 fps |
| 1000 m z17 ×5 p60（pitch 60.00・タイル 17） | 60.0 fps | 60.0 fps | 60.0 fps | 60.0 fps |
| 1000 m z16 ×10 p85（pitch 78.60・タイル 16） | 59.6 fps | 51.0 fps（D15 を外す） | 60.0 fps | 60.0 fps |

- 列の違い: 51.0（49.0〜55.8）は c25be04（Task 9、矢印の既定 10 m・10,000 本）で測った値。Task 10（9079cba）が 1000 m の既定を 20 m（2,500 本）にした後は測り直しておらず、05 の最終 b3a8916 では同じ条件で 60.0（06 の M1 で実測、`.handoff/06-perf/baseline-check.md`。視点・描かれるタイルは同じ）。地形のみも 59.6 → 60.0。A/B では c25be04 が 55.3、c25be04 に 9079cba だけを重ねると 59.9 で、1000 m の矢印の本数の差で戻る
...
- 06 へ引き継ぐ条件: 05 の最終では 8 条件すべてが D15 を満たす。残るのは、矢印の間隔に 5 m を選んだとき（1 辺 100 本・10,000 本。`arrowSpacingForRange(baseM, sizeM) = baseM × sizeM ÷ 500` により範囲によらない本数。c25be04 の 1000 m の既定と同じ本数）の落ち方。**06 の M3（2026-09-17、`docs/perf/2026-09-17-isolation.md`）で計測**: 1000 m は 55.7（49.1〜57.1）fps・g2 41（29〜107）で D15 を外し、見込みどおり c25be04 の 55.3 に近い値に落ちた。500 m は 58.3（55.0〜59.4）fps・g2 17 で中央値は D15 の内（落ち方は 1000 m より小さい。1 フレームの予算〈16.7 ms〉の見積もりでは、矢印 10,000 本のコストは範囲によらず約 +9.5 ms、水面の上乗せは 1000 m が約 +2.4 ms・500 m が約 +0.6 ms〈水深テクスチャが約 1/4〉で、合計は 1000 m ≈ 19.7 ms・500 m ≈ 17.9 ms と見積もる。推定、未検証。`docs/perf/2026-09-17-isolation.md`「500 m での確かめ」）。矢印なし・矢印 20 m（625 本）・既定（2,500 本）はいずれも 60.0 前後で、fps は矢印の本数とともに下がるが既定までは 60.0 に張り付き、2,500 本と 10,000 本の間で急に落ちる（主因は矢印の symbol の本数。転送・水面の描画・地形の描画は既定の本数では上限の内）。タイルの組み立て（24 ラン全部を見ても最大 15.7 ms）と水面の render CPU（0.416 ms）は、05 の記録の時点で原因から外れている

06 の着手時の実測（2026-09-17、実 GPU・headless・開発機〈AMD Ryzen 9 5950X 16 コア 32 スレッド、NVIDIA GTX 1080 Ti〉。`docs/perf/2026-09-17.md`。1 step と平衡は固定の DEM で、段 2 の DEM で走ったみなとみらい 1000 m を除く。平衡の判定は暫定で、06 の M4 の後に確定する）:

| 指標 | 目標 | 測った値（最悪の条件） | 判定 |
|---|---|---|---|
| 1 step の所要時間（中央値） | < 8ms | 9.50 ms（綾瀬 500 m・全面を濡らす雨・2D。渋谷 8.80・みなとみらい 9.10。1000 m は 37.30・44.60 ms） | 超過 |
| 1 step の所要時間（p95） | < 16ms | 13.30 ms（みなとみらい 500 m・全面を濡らす雨・2D。1000 m は 54.50・56.10 ms） | 基準内（500 m） |
| 地図操作時のフレームレート | D15 | 3 地点 × 4 視点 × 地形のみ・水面ありの 24 セルすべて 60.0 fps（ランの最小 59.8）、長いフレーム 0、g2 最大 2。雨は 500 mm・半径 50 m、矢印は既定の本数。みなとみらい 1000 m は段 2 | 基準内（既定の本数）。**06 の M3（2026-09-17）で測定**: 矢印 5 m を選んだとき（1 辺 100 本・10,000 本。範囲によらない）、1000 m は 55.7（49.1〜57.1）fps で D15 を外すが、500 m は 58.3（55.0〜59.4）fps で中央値は D15 の内。主因は矢印の symbol の本数。**R06-11 の裁定 (a2)**: 全範囲で矢印の 1 辺の本数の上限を 50 にする（5 m の選択肢を外す）。M4 の Task 13b で実装し、前後を測った（1000 m 55.5 → 60.0、500 m 53.2 → 60.0 fps。`docs/perf/2026-09-17-fixes.md`） |
| 地点クリックから 2D の地形表示まで | < 3秒 | 0.77 s（開発機の回線。綾瀬 1000 m の 3 回の中央値。1 ランの最大 0.82 s） | 基準内 |
| 3D に切り替えてから最初の 3D のフレームまで | < 3秒 | 0.64 s（渋谷 1000 m の 3 回の中央値。1 ランの最大 0.67 s） | 基準内 |
| メインスレッドの最長ブロック時間 | < 50ms | 206 ms（綾瀬 1000 m の 3D に切り替え）。1000 m の読み込みの終わり 137 ms、500 m の読み込み 73 ms | 超過 |
| 平衡までの時間 | 半径 10m は 60 秒、半径 100m は 5 分 | 半径 10m: 87.7 s（綾瀬 500 m。渋谷 45.4 s・みなとみらい 2.9 s）。半径 100m: 500 m の 3 地点とも 300 秒で届かず | 暫定: 超過 |

1番目と2番目の未達が §6.3 の WASM 移行検討の条件となる。

06 の M4 の後（2026-09-17、同じ機体。M4 = 安価な改善〈Task 14〜19〉と平衡の測り直し〈Task 25〉。`docs/perf/2026-09-17-fixes.md`「M4 の後の判定」）:

| 指標 | 目標 | 測った値（最悪の条件） | 判定 |
|---|---|---|---|
| 1 step の所要時間（中央値） | < 8ms | 7.90 ms（綾瀬 500 m・全面を濡らす雨・2D・`until=settle`〈M2 と同じ条件、Task 25 の実測〉。渋谷 6.50・みなとみらい 7.30。M2 比 −16.8%〜−26.1%。Task 15 自身の前後〈同日、`until=window` の別の計測〉は 8.60・8.20・8.30 ms で −17.3%〜−18.8%、M2 比とは基準が異なるため比べない。1000 m は参考で 30.80・36.00 ms、M2 比 −17〜−19%） | 基準内（500 m）。1000 m は超過（参考。1000 m 四方の行を見る） |
| 1 step の所要時間（p95） | < 16ms | 11.10 ms（綾瀬 500 m・全面を濡らす雨・2D・`until=settle`。渋谷 9.60・みなとみらい 10.70。いずれも 16 ms の内。1000 m は参考で 46.50・47.30 ms） | 基準内（500 m）。1000 m は超過（参考） |
| 地図操作時のフレームレート | D15 | 矢印の 1 辺の本数の上限を全範囲で 50 にした後（Task 13b）: 1000 m 矢印 5 m 60.0（59.9〜60.0）fps・g2 0（0〜1）、500 m 矢印 5 m 60.0（60.0〜60.0）fps・g2 0（0〜0）。既定の矢印の本数は前後とも 60.0 | 基準内（Task 13b で直した。前: 1000 m 55.5・500 m 53.2 fps → 後: 1000 m 60.0・500 m 60.0 fps） |
...
| 1000m 四方 | 上の基準をすべて満たすか | M4 で最長ブロック・止まっている間の再描画・再生中の割り当て・fps は基準内になった。なお外れるのは 1 step（参考、段 1〈DEM1A〉の`until=settle`実測で綾瀬 30.80・渋谷 36.00 ms）と平衡（綾瀬 r10 80.2 s、r100 は綾瀬・渋谷とも届かず） | **ユーザー裁定（2026-09-18、M6・Task 29）**: 1000 m の選択肢は残す。ただし注意書きを出す（Rust WASM の spec は起こさない）。1 step・平衡の両方が 500 m にも及ぶ問題（fill-spill-merge の対応に集約）と、1000 m 固有の 1 step の重さとが混ざっているため、1000 m だけを外す理由にはならないと判断した。**Task 29b（ab5bdce）で実装**。文言『範囲 1000 m は動作が重くなることがあります。』を、1000 m を選んだときだけ範囲の選択の下に出す |
...
06 の着手時（2026-09-17、`pnpm build && pnpm size`。`docs/perf/2026-09-17.md`）:

| チャンク | 実測 | 予算 |
|---|---:|---:|
| `map` | 246.6 KB | 260 KB |
| `ui` | 160.0 KB | 150 KB（超過。04 からの持ち越し。06 の M4 で直す） |
| `index` | 20.5 KB（別に rolldown のランタイム 0.4 KB） | 20 KB（超過） |
| 初期ロードの合計 | 427.5 KB | 500 KB |
| `maplibre-gl-worker` | 131.8 KB | — |
| `three` | 126.5 KB | — |
| `simulation.worker` | 8.8 KB | — |
| `View3d` | 5.8 KB | — |
| `waterLayer` | 2.1 KB | — |
| 総量 | 702.4 KB | 1.2 MB |

- 初期ロードと `map`・`ui`・`index`・`maplibre-gl-worker`・`three`・`View3d`・`waterLayer` は 05 と同じ。`simulation.worker` が 8.7 → 8.8 KB、総量が 702.2 → 702.4 KB（06 の M1 の Worker の変更のぶん。計測のコードは通常のビルドに残らないことを確かめている）
- `maplibre-gl-worker` のチャンクは `maplibre-gl-shared.mjs` と `maplibre-gl-worker.mjs` の 2 モジュールだけで、`map` のチャンクと共通するのは `maplibre-gl-shared.mjs` の 1 件。これは MapLibre 自身の Worker が持つ複製で、アプリの設定の誤りではない（実装 spec 06 の計画で決めたこと 18）。記録に留める

06 の M4 の後（Task 19 の後、2026-09-17、`pnpm build && pnpm size`。実装 spec 06 の計画 Task 25 で確認）:

| チャンク | 実測 | 予算 |
|---|---:|---:|
| `map` | 246.6 KB | 260 KB |
| `ui` | 151.8 KB | 150 KB（Task 19 で 160.0 → 151.8 KB。なお 1.8 KB 超過。RB-1 の次の手〈パネルの遅延読み込み〉は M6） |
| `index` | 21.0 KB（別に rolldown のランタイム 0.4 KB） | 20 KB（超過） |
| 初期ロードの合計 | **419.8 KB**（Task 19 で −8.1 KB） | 500 KB |
| `maplibre-gl-worker` | 131.8 KB | — |
| `three` | 126.5 KB | — |
...
| `jumpTo` に center・zoom を渡すと `terrain.getElevationForLngLatZoom(center, options.zoom)` で中心の標高を決める（22264 行）。小数のズーム（例: 15.924）では `_getOverscaledTileIDFromLngLatZoom`（10549〜10559 行）が `zoom` をそのまま `OverscaledTileID` の `z` に使ってタイル ID を作るため、整数のズームで読み込んだタイルと一致せず 0 m になる。`jumpTo` の末尾の `applyUpdatedTransform`（22452〜22470 行）は常に `_elevateCameraIfInsideTerrain`（22431〜22443 行、22454 行で登録）をカメラにも適用し、同じ `getElevationForLngLatZoom` で `minAltitude` を読むため、小数のズームではここも 0 m になり、カメラが地形の中に入っていても持ち上げない。中心の標高は zoom を渡さない次の `jumpTo`（`tr.tileZoom` を使う。整数）で直るが、`View3d.frame` の `easeTo`（`View3d.ts:330`）は `freezeElevation` オプションを渡さないため `_finalizeElevation`（22371・22402 行）が呼ばれず `elevationFreeze` が立ったままになり、`_render`（26181 行）とタイル読み込み時（25066〜25068 行）の毎フレームの標高の再計算が両方止まる（実アプリではドラッグ・ズームの終わりで handler が `elevationFreeze` を戻す〈22006 行〉ので、中心の標高への影響はない） | `src/ui/perfWater.ts` の `settleCenterElevation`（中心の標高）。`placeViewOnLoadedTerrain` を使うほかの計測（fps・view）の視点も同じ影響を受ける。カメラの標高が 0 m のまま持ち上がらない件は p85 の PoC（M5 レビュー R2、ユーザー裁定 5〈2026-09-18〉）で、ホイールのズームは小数なので実アプリでも起きる | 中心の標高は計測だけ（実アプリでは操作の終わりに `elevationFreeze` が戻るため影響しない）／カメラの守り（`_elevateCameraIfInsideTerrain`）は本番でも `tr.zoom` が小数（ホイールのズーム）のときは効かない（ユーザー裁定 (5)、2026-09-18） | 中心の標高が 0 m に置かれるか（直っていれば `settleCenterElevation` は何もしない）。カメラが地形の中に入ったまま持ち上がらないままか |
